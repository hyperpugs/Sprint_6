...
from locators import LocatorsBase, LocatorsMain
...
class BasePage:
    ...
    def _open_the_page(self):
        self.driver.get(LocatorsMain.URL)
    ...
    def click_on_order_button_in_header(self):
        self._wait_and_click_on_element(LocatorsBase.BUTTON_ORDER_IN_HEADER)

    @allure.step('Нажать на лого "Самокат" ')
    def click_on_logo_scooter(self):
        self._wait_and_click_on_element(LocatorsBase.LOGO_SCOOTER)

    @allure.step('Нажать на лого "Яндекс" ')
    def click_on_logo_yandex(self):
        self._wait_and_click_on_element(LocatorsBase.LOGO_YANDEX)
    ...
    def check_is_it_main_page(self):
        assert EC.url_to_be(LocatorsMain.URL), f'Переход не на главную страницу'
